...
namespace DECOMP {
	...
	DECOMP::SOM SOM::dCdt( double T, double wetness, double pH, double Nsol ) const
	{
		SOM dispatch;
		SOM decomp;
		const SOM& self = *this;
	...
	DECOMP::SOM SOM::operator*( double right ) const
	{
		SOM res=*this;
		res*=right;
		return res;
	...
	std::string SOM::to_string() const
	{
		...
		sstr.precision(4);
		sstr << "SOM(N=" << N;
		for(DECOMP::component_set::const_iterator it = SOM::pool_types.begin(); it != SOM::pool_types.end(); ++it)
		{
	...
	DECOMP::component_set& SOM::get_pool_types()
	{
		return DECOMP::SOM::pool_types;
	}

	DECOMP::SOMcomponent SOM::add_component( std::string name, bool is_stored,double k_pot, double E_a, double K_w, double n_w, double K_pH, double m_pH )
	{
	...
	DECOMP::component_set SOM::pool_types=DECOMP::init_SOMcomponents();

	DECOMP::SOM wood_litter()
	{
		return SOM(1./50., .04, .6, .27,.09);
	}

	DECOMP::SOM leave_litter()
	{
		return SOM(1./50., .1,.5,.32,.08);
	}

	DECOMP::SOM root_litter( )
	{
